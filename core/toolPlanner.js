...
async function planLucyActionWithDeepSeek({ userText = "", memory = {}, availableTools = [] } = {}) {
  ...
  const systemPrompt = [
    ...
    "Kritik semantik kural: Kullanıcı 'bunu X yap' derse önce X'i sınıflandır: renk/stil mi, grafik tipi mi, tablo mu, dosya formatı mı, içerik işlemi mi?",
    "X renk/stil ise mevcut çıktının tipini koru; grafikse chartData modify planla, renk tablosu/HTML/metin üretme.",
    "X grafik tipi ise son veri/grafik üzerinden istenen chartType'a dönüştür; X dosya formatı ise export tool planla.",
    "Renkler hardcode değil: kullanıcı hangi renk/tema/ton yazdıysa dinamik çıkar ve input.style/rawStyle veya input.colors ile aktar.",
